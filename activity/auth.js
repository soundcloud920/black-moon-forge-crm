const ACTIVITY_UNLOCK_KEY = "bmf-activity-unlock-v1";
const ACTIVITY_SESSION_KEY = "bmf-activity-session-v1";
const ACTIVITY_DEVICE_KEY = "bmf-activity-device-v1";
...
unlockForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    storage.setItem(ACTIVITY_UNLOCK_KEY, JSON.stringify(unlocked));
    mountApp(unlocked);
  } catch {
    unlockError.textContent = "Не удалось открыть Activity CRM.";
  }
});
...
function readUnlock() {
  const raw = localStorage.getItem(ACTIVITY_UNLOCK_KEY) || sessionStorage.getItem(ACTIVITY_UNLOCK_KEY);
  if (!raw) return null;
  try {
    const saved = JSON.parse(raw);
    if (saved.payloadBuild !== currentPayloadBuild()) {
      localStorage.removeItem(ACTIVITY_UNLOCK_KEY);
      sessionStorage.removeItem(ACTIVITY_UNLOCK_KEY);
      return null;
    }
    return saved;
  } catch {
...
async function unlockPayload(login, password) {
  const envelopes = window.BMF_ACTIVITY_PAYLOAD?.envelopes || [];
...
function mountApp(unlocked) {
  localStorage.setItem(ACTIVITY_SESSION_KEY, JSON.stringify(unlocked.session));
...
function getDeviceId() {
  let id = localStorage.getItem(ACTIVITY_DEVICE_KEY);
  if (!id) {
...
function currentPayloadBuild() {
  return String(window.BMF_ACTIVITY_PAYLOAD?.buildId || "");
}
